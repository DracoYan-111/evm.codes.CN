...
在组$G_1$中，基域$F_p$上的点由(x,y)坐标组成，其中每个坐标被编码为32字节。

在组 $G_2$ 中，扩展域 $F_{p^2}$ 上的点用坐标 (x, y) 表示，其中 `x = x_c0 + x_c1 *i` 和 `y = y_c0 + y_c1 *i`，每个分量 (x_c1, x_c0, y_c1, y_c0) 编码为 32 个字节。

无穷远点用字段“x”和“y”编码为“0”。
## Inputs

输入必须始终是 6 个 32 字节值的倍数。 0 个输入有效并返回 1。一组输入定义如下：
| 字节范围 | Name |
|-----------:|-----:|
| `[0; 31]` (32 bytes) | g1_x |
| `[32; 63]` (32 bytes) | g1_y |
